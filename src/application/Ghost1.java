...
public class Ghost1 extends Ghost {
	...
	/**
	 * default constructor for Ghost1 object
	 */
	...
	/**
	 * @param row, col, g, c, v
	 * @return coordinates to where the ghost should move based on the grid layout of the maze and movement determined
	 */
	public GhostCoordinates move(int row, int col, GridPane g, char[][] c, boolean v) {
		...
		// if vulnerable the ghosts gif switches to blue
		if (v) {
			...
			if (c[row][col + 1] != 'W') {         // W represents a wall
				if (c[row][col + 1] == 'L') {     // L represents a pellet
					...
					if (c[row][col] == 'X') {     // X is the path pacman took
						...
						c[row][col + 1] = '1'; //updates position on maze
						c[row][col ] = 'X'; 
